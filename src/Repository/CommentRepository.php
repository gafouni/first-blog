...
class CommentRepository extends CoreRepository{
    ...
    public function findAllByPost($post): array{
        $comments=[];
        $pdo_st=$this->pdo->prepare('select * from `comment` where post_id = :post_id order by `date` desc');
    ...
    public function create(Comment $comment){
        $pdo_st=$this->pdo->prepare("INSERT INTO `comment` (`date, `content`, `name`, `email`, `post`) VALUES (now(), :content, :name, :email, :post)");
